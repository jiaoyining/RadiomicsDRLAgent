...
import time
import radiomicscpu as radiomics
...
def get_radiomics_descriptor_for_image(image, yaml='./Params.yaml'):
    #radiomics_features = []
    features = []
    for i_image in image:
        mask = np.ones(i_image.shape)
        extractor = radiomics.featureextractor.RadiomicsFeaturesExtractor(yaml)
        sitk_image = sitk.GetImageFromArray(np.expand_dims(i_image[:, :, 0], 0))
        sitk_mask = sitk.GetImageFromArray(np.expand_dims(mask[:, :, 0], 0).astype('int16'))
        result = extractor.execute(sitk_image, sitk_mask)
        count = 0
        for key, value in six.iteritems(result):
            if count > 1:
                features.append(value/(abs(value)+1.0))
            count += 1
...
def get_image_descriptor_for_image(image, model):
    ...
    #dim_ordering = krs.backend.imageimage_dim_ordering()
    dim_ordering = 'tf'
    if dim_ordering == 'th':
        # 'RGB'->'BGR'
        im = im[::-1, :, :]
        ...
        # 'RGB'->'BGR'
        im = krs.applications.vgg16.preprocess_input(im)
        '''
        # Zero-center by mean pixel
        im[:, :, 0] -= 103.939
        im[:, :, 1] -= 116.779
    ...
    im = im.transpose((2, 0, 1))
    im = np.expand_dims(im, axis=0)
    inputs = [krs.backend.learning_phase()] + model.inputs
    convout1_f = krs.backend.function(inputs, [model.layers[18].output])
    #krs.applications.vgg16.decode_predictions()
    return convout1_f([0] + [im])
...
def get_conv_image_descriptor_for_image(image, model):
    ...
    conv_maps = []
    for i_image in image:
        im = cv2.resize(i_image, (224, 224)).astype(np.float32)
        #dim_ordering = K.image_dim_ordering()
        dim_ordering = 'tf'
        if dim_ordering == 'th':
            # 'RGB'->'BGR'
            ...
            # 'RGB'->'BGR'
            im = np.expand_dims(im, axis=0)
        conv_maps.append(model.predict(im)[0])
...
def obtain_compiled_vgg_16(vgg_weights_path):
    ...
    #sgd = krs.optimizers.SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
    #model.compile(optimizer=tf.train.AdamOptimizer(0.1), loss='categorical_crossentropy')
    return model
...
def vgg_16(weights_path=None):
    ...
    '''
    model = krs.applications.VGG16(weights='imagenet', include_top=False)
